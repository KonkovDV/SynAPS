...
- 22 публичные конфигурации решателей в реестре (`available_solver_configs()`)
...
- Свежий аудит current-head до обновления профиля: `benchmark/studies/2026-04-27-rhc-50k-audit-v1/rhc_50k_study.json`

Сводка по свежему аудиту current-head до обновления профиля:
...
- Свежий аудит 2026-04-27 зафиксировал третий режим и опроверг слишком сильное старое чтение «ALNS вообще не входит в search»: ранние окна действительно входят в ALNS, но окна 2-3 тратят 100 repair attempts на нулевую по полезности CP-SAT micro-repair, а поздние окна уходят в fallback или timeout из-за hybrid CP-SAT routing.
- Именно поэтому публичные defaults для `RHC-ALNS` теперь обновлены: hybrid CP-SAT routing и CP-SAT micro-repair выключены. Артефакты 2026-04-26 и 2026-04-27 теперь нужно читать как evidence отказа старого CP-SAT-heavy профиля, а не как performance-утверждение для обновлённого default.
...
- Fresh current-head `RHC-ALNS|throughput` в `2026-04-27-rhc-50k-audit-v1` даёт `mean_scheduled_ratio = 0.1243`, `mean_makespan_minutes = 4134.84` и `mean_inner_fallback_ratio = 0.625`.
- Post-fix `RHC-GREEDY|throughput` из `test-50k-after-fix` остаётся более сильным базовым режимом максимального покрытия с `mean_scheduled_ratio = 0.37` и `mean_inner_fallback_ratio = 0.0`.
- Нерешённая исследовательская задача теперь распадается на две части: старый публичный профиль терял бюджет на CP-SAT side-paths при 50K, а выше 100K+ отдельно остаётся узкое место в initial seed generation.
...
- Этот staged `v3` результат опровергает старое чтение вида «на 100K ALNS вообще не доходит до search»: контролирующим узким местом оказались geometry первого окна и давление initial seed generation, а не только более поздний CP-SAT repair flag. Но и это ещё не production-readiness, потому что bounded run остаётся частичным (`mean_scheduled_ratio = 0.0468`, `feasible = false`).

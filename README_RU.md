...
- Нерешённая исследовательская задача теперь распадается на две части: обновлённый публичный 50K path больше не теряет бюджет на CP-SAT side-paths, но всё ещё проигрывает поздние окна из-за seed-construction exhaustion; `100k+` остаётся отдельным bottleneck по coverage.
...
- Но и это всё ещё не production-readiness: bounded current-head run остаётся частичным (`mean_scheduled_ratio = 0.0342`, `feasible = false`) и всё ещё уступает same-run greedy baseline по scheduled coverage.
- Значит, следующий жёсткий engineering boundary теперь раздваивается: `300k` и `500k` режутся model/schema capacity, а `100k` и `200k` всё ещё требуют лучшего seed/admission/coverage yield без возврата к тяжёлым CP-SAT side-paths.

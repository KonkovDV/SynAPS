...
- Последний после исправлений артефакт с защитными ограничителями: `benchmark/studies/2026-04-26-rhc-alns-postfix-canonical-v4/rhc_50k_study.json`

Сводка по последнему артефакту после исправлений с защитными ограничителями:
...
- Pre-fix stress-matrix остаётся честной поверхностью подтверждений для случая, где ALNS давал лучший partial objective при слабом покрытии.
- Последний после исправлений артефакт показывает уже другой режим: oversized ALNS-окна теперь явно short-circuit'ятся до дорогой phase-1 seed generation, чтобы не сжигать весь бюджет окна.
- На `industrial-50k` это заметно повышает пропускную способность относительно неудачных повторных запусков после исправлений (`mean_scheduled_ratio` восстановился до `0.3028`), но ценой того, что ALNS вообще не входит в destroy-repair search на этих окнах.
- Поэтому текущую линию пропускной способности `RHC-ALNS` нужно читать как контроллер резервного режима с защитными ограничителями, а не как доказательство эффективности large-window ALNS search на 50K.
...
- Нерешённая исследовательская задача теперь локализована: нужна такая window geometry и routing policy, при которой ALNS реально успевает войти в search, а не сгорает на phase-1 seed generation или принудительно уходит в fallback.

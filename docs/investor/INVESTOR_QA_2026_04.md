...
The universal schema and domain examples are working. The Python solver, both heuristic and exact, passes 27 out of 27 current tests. The benchmark harness runs, and my CP-SAT solver is 42% faster than the greedy baseline in smoke tests. The documentation stack is published too: literature review, benchmark protocol, architecture docs, plus the 27-document investor pack you're reading right now.
...
I deliberately avoid citing Gartner, Mordor Intelligence, or IDC — their reports are paywalled and unverifiable. The reasoning is in [ACADEMIC_METHODS_APPENDIX](ACADEMIC_METHODS_APPENDIX_2026_04.md) §6.

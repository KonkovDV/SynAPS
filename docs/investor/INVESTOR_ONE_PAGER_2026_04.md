...
The universal schema and domain examples are working. The Python solver, both heuristic and exact, passes all 27 current tests. I have a benchmark harness: in a smoke run, the CP-SAT solver produces schedules 42% faster than the greedy baseline (139.67 min → 81.0 min). There's a published literature review, benchmark protocol, and architecture docs. And the investor pack you're reading — 27 documents, every claim tagged by evidence level.

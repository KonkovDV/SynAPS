...
(`tests/test_determinism_override_rejected.py`). C7 closes ingest EST
ceil, LFT floor (C7-R1), `random_seed` deny under `strict`, and
`objective_sort_key` / Pareto pick via `scalarize()`.
...
| **C7-P5** | P3 | `latest_finish` not floored at ingest (C7-R1) | Ingest floors; greedy/CP-SAT use `operation_latest_finish_offset_minutes`; 90s finish vs 1-minute LFT is `HORIZON_BOUND_VIOLATION` |
...
| `due_date` at +90s is ceiled or floored (tardiness shift) | **blocked** — EST/LFT fields only |
| Ceil `latest_finish` (90s → 2 min) | **blocked** — that relaxes the deadline; floor is 1 min |
...
feasibility relaxation, ingest LFT floor is a feasibility relaxation
of the declared deadline (it tightens), C5a, delta-notary default.

...
- CP-SAT/ALNS search may take `CABLE_PVC_CPSAT_WEIGHTS` / `CABLE_PVC_WEIGHTS`. GREEDY/COVER construction is unchanged. C6c: `--weighted-residual` seeds ALNS from COVER; 1600@8 PVC residual cut tardiness 25–478 min vs cover (not ATP). C6-R1: `--waves 4` at 8/stage is **plumbing** (stop-on-dirty, wave notary kinds). Seed 2 was `INFEASIBLE` once on weeks 3–4; Hamming is path-dependent. Not a freeze-quality proof.

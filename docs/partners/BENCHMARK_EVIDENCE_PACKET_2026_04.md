...
date: "2026-04-04"
tags: [synaps, benchmark, evidence, partner]
mode: "reference"
---
...
Date: 2026-04-04
Status: active
Scope: partner-facing benchmark evidence policy and current benchmark proof state for SynAPS
...
1. the active evidence pack preserves a historical `149/149` full-suite pytest pass artifact, while the current repository now collects `175` tests across 26 modules;
...
These results are recorded in `TECHNICAL_VERIFICATION_REPORT_2026_04.md`, which now distinguishes the current 175-test collection boundary from the last published historical full-suite pass artifact.
...
1. publish medium-instance repeated runs for `GREED`, `CPSAT-10`, and `CPSAT-30`;
...
5. keep the benchmark packet linked from the partner diligence router as a bounded smoke-evidence surface until stronger data exists.
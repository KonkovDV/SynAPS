...
- `NOT CLAIMED`: should not be promoted into the active partner narrative
...
| SynAPS currently ships ten verified runtime components across its deterministic solver, routing, and validation surfaces: **CP-SAT** exact solver (772 LOC), **LBBD** decomposition (969 LOC), **LBBD-HD** parallel decomposition (1 324 LOC), **Greedy ATCS** dispatch (296 LOC), **Pareto Slice** $\varepsilon$-constraint (104 LOC), **Incremental Repair** (318 LOC), **Portfolio Router** (252 LOC), **Graph Partitioning** (271 LOC), **Solver Registry** (210 LOC), and **FeasibilityChecker** 7-class validator (280 LOC). Aggregate LOC for these current solver/routing/validation surfaces: 4 796 | CURRENT CORE | C2 | `synaps/solvers/`, `synaps/portfolio.py`, `tests/`, `TECHNICAL_VERIFICATION_REPORT_2026_04.md` | one of the named solver paths stops being runnable or verified | repeated benchmark packet over larger instance families |
...
When adding any new partner-facing statement, answer five questions first:
...
If those answers are missing, the claim should not be promoted into the active partner packet.

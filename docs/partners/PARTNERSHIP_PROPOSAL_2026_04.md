---
title: "SynAPS Partnership memo 2026-04"
status: "active"
version: "2.0.2"
last_updated: "2026-04-07"
date: "2026-04-05"
tags: [synaps, partner, proposal, startup]
mode: "reference"
---

# Partnership memo: SynAPS
...
The deterministic layer makes sure every plan is physically feasible - the math won't produce a schedule that violates real constraints. The architecture leaves room for a bounded advisory layer above that kernel, but the shipped repository proof today is still the deterministic scheduler plus routing and repair paths. Advisory ML or LLM behavior remains a target layer rather than a current runtime surface.
...
The planning engine handles precedence, capacity limits, resource occupancy, tooling availability, and physical feasibility. Every output schedule passes a constraint check before it goes anywhere. No exceptions. The architecture is intentionally prepared for a bounded advisory layer above that feasible solution space - a layer that could later rank alternatives or explain tradeoffs. In today's repository, execution itself remains deterministic.
...
I want to be precise here. SynAPS in its current state is an engineering thesis backed by a working architectural-mathematical formalization. It's not a deployed APS runtime - I won't pretend otherwise. The strongest asset right now isn't product KPIs. It's a well-formulated kernel with universal economic logic across manufacturing domains, an explicit mathematical formulation, and a clear trajectory from the current heuristic core to an academically validatable hybrid stack.

The universal schema works - DDL plus domain examples. The repository currently collects 175 tests across 26 modules around the solver surfaces, while the newest fully recorded full-suite pass artifact preserved in the active evidence packet is still an earlier `149/149` snapshot, so a fresh `175/175` pass artifact has not yet been published. I've also closed SDST parity for exact solve, repair, and feasibility checking. The benchmark harness supports a verified smoke-instance result on `tiny_3x3`: `CPSAT-10` improves makespan from `106.67` to `82.0` minutes, or about `23.1%` versus `GREED`. Broader benchmark coverage remains open. Research documentation is published.
...
The April 2026 verification pass came back clean on the current documented scope: technical checks passed, the benchmark smoke path is live, and the partner evidence pack is internally consistent.
...
**C2 - internal proof exists:** problem formalization, architecture, universal schema, solver baseline, benchmark harness, research roadmap. An partner can verify each of these directly.
...
2. [Partner_ONE_PAGER](PARTNER_ONE_PAGER_2026_04.md) - fastest summary
3. [Partner_DILIGENCE_PACKET](PARTNER_DILIGENCE_PACKET_2026_04.md) - diligence entry point

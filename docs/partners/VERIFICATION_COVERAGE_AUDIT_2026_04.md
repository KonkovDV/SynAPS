...
date: "2026-04-05"
tags: [synaps, verification, tests, audit, partner]
mode: "evidence"
---
...
Date: 2026-04-04
Status: active
Scope: fresh verification snapshot for the current SynAPS partner and technical-diligence surfaces across project-level and core-technical proof paths (updated after Phase 2 code hardening and CI supply-chain fixes)
...
| --- | --- | --- |
| Python `pytest` | HISTORICAL PASS ARTIFACT + CURRENT COLLECTION REFRESH | active evidence pack still preserves an earlier `149/149` full-suite pass artifact; current repository collection now reaches `175` tests across 26 modules and still needs a newly published `175/175` pass artifact |
...
| Check | Fresh result | Evidence basis |
| --- | --- | --- |
| optional `docs/partners/` subtree removal rehearsal | PASS | code, build, benchmark, and export checks stayed green without the subtree |
...
5. the partner subtree is optional rather than a runtime dependency;
...
2. the current partner-safe supply-chain story is still partial even though release attestations and SBOM surfaces now exist;
...
4. the suite has expanded to 175 collected tests, but this snapshot does not yet contain a newly published `175/175` full-suite pass artifact.

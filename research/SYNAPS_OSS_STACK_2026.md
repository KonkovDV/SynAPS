...
| --- | --- | --- |
| Google OR-Tools (CP-SAT) | `>=9.10` в `pyproject.toml` | Точный solver для Job-Shop Scheduling. 120/120 тестов проходят |
...
CP-SAT (Google) остаётся абсолютным лидером для задач MO-FJSP-SDST-ARC в Open Source. HiGHS — самый быстрый открытый LP/MIP solver, заменивший Coin-OR (CBC). Эта связка — текущий production-стек SynAPS.
...
* **Flower (v1.10+)** — лидер Open Source для FL. Дифференциальная конфиденциальность, безопасная агрегация.

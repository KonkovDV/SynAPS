...
| KI-F7 | LOW | closed (envelope) | Exact lane inference holds for n≤512 / max_parallel≤8 / budget OK; greedy fallback emits advisory `LANE_INFERENCE_UNPROVEN` (filter via `hard_violations`). **Residual (RT17-H2):** portfolio `verified_feasible` still uses raw `violations` and can false-fail greedy-trap schedules outside the exact envelope — Wave 8. | `tests/test_exact_lane_inference.py::test_greedy_fallback_emits_lane_inference_unproven` | F7 / Wave 5 / RT17 |

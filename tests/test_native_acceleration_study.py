...
def test_native_acceleration_study_writes_report(tmp_path: Path) -> None:
    output_path = tmp_path / "native-atcs-report.json"

    subprocess.run(
        [
            sys.executable,
            "-m",
            "benchmark.study_native_acceleration",
            "--sizes",
            "64",
            "--repeats",
            "1",
    ...
    assert result_modes == {"scalar_python", "batch_python", "batch_active"}

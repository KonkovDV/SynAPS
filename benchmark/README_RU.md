...
Последний pre-fix live stress-matrix артефакт лежит в [studies/test-50k-academic-matrix-v1/rhc_50k_study.json](studies/test-50k-academic-matrix-v1/rhc_50k_study.json).

Последний post-fix guarded-артефакт лежит в [studies/2026-04-26-rhc-alns-postfix-canonical-v4/rhc_50k_study.json](studies/2026-04-26-rhc-alns-postfix-canonical-v4/rhc_50k_study.json).

Их нужно читать вместе, а не схлопывать в одну историю:

- pre-fix stress-matrix сохраняет старый профиль деградации ALNS, где покрытие было слабым, но часть окон всё же входила в search;
- post-fix guarded-артефакт фиксирует новое operational rule: oversized ALNS-окна теперь пропускаются до дорогой seed generation, чтобы не сжигать весь бюджет окна;
...
- pre-fix `RHC-ALNS|throughput` давал `mean_scheduled_ratio = 0.0946`, `mean_makespan_minutes = 4985.85` и `mean_inner_fallback_ratio = 0.1`;
- post-fix guarded `RHC-ALNS|throughput` даёт `mean_scheduled_ratio = 0.3028`, `mean_makespan_minutes = 9675.18` и `mean_inner_fallback_ratio = 1.0`;
- post-fix `RHC-GREEDY|throughput` остаётся более сильным pure-coverage baseline с `mean_scheduled_ratio = 0.37` и нулевым inner fallback.

Следовательно, публичный 50K path по-прежнему реален и воспроизводим, но последнее evidence говорит не «ALNS исправлен», а нечто более узкое: репозиторий теперь предотвращает pathological ALNS pre-search budget burn на oversized окнах, однако throughput-линия 50K всё ещё не имеет такой window geometry, при которой ALNS реально входит в destroy-repair search и обгоняет guarded fallback по coverage.

...
Python bridge теперь прокидывает только минимально необходимое окружение:
системные path/process переменные, `SYNAPS_PYTHON_*`, `SYNAPS_DISABLE_NATIVE_ACCELERATION`
и `OTEL_*`. Control-plane-only настройки вроде `SYNAPS_CONTROL_PLANE_API_KEY`
и параметров rate-limit/body-limit намеренно не передаются в дочерний Python-процесс.
...
Если runtime-caller хочет не default balanced routing, а coverage-oriented режим, передавайте
`context.portfolio_policy = "feasibility-first"`. Control plane валидирует и прокидывает этот
policy без изменений, а Python router затем смещает nominal non-exact flows в сторону
feasible-coverage portfolio members вместо exactness-heavy defaults.
